...
namespace WebAppAPICrud.Controllers
{
    ...
    public class UserSearchController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<User>>> GetUserSearch(string? city = "", string? gender = "", int? before = null, int? after = null, DateTime? time = null)
        {
            ...
            if (gender != "" && city == "" && before == null && after != null && time == null)
            {
                var users = await _context.Users.Where(x => x.Gender == gender && x.Age <= after).ToListAsync();
                return Ok(users);
            }
            ...
            if (gender == "" && city != "" && before != null && after == null && time == null)
            {
                var users = await _context.Users.Where(u => u.City == city && u.Age >= before).ToListAsync();
                return Ok(users);
            }
            ...
            if (gender == "" && city == "" && before != null && after == null && time == null)
            {
                var users = await _context.Users.Where(u => (u.Age >= before)).ToListAsync();
                return Ok(users);
            }
            //вораста до
            if (gender == "" && city == "" & before == null && after != null && time == null)
            {
                var users = await _context.Users.Where(u => (u.Age <= after)).ToListAsync();
                return Ok(users);
            }
            //от и до
            if (gender == "" && city == "" & before != null && after != null && time == null)
            ...
            {
                var users = await _context.Users.Where(
                    u => u.City == city && u.Gender == gender && (u.Age >= before && u.Age <= after) && u.DataRegistration == time).ToListAsync();
